...
from src.gaussian_volume._3dgs_training import train_impl as _train_impl
...
def _load_3dgs_kernel():
    ...
    Sources live in csrc/training/: common.cuh (shared device helpers),
    forward.cu, backward.cu, regularization.cu (one kernel + entry point
    each), and bindings.cpp (pybind11 module definition only).
    """
    global _3dgs_cuda
    if _3dgs_cuda is None:
        csrc_dir = Path(__file__).parent / "csrc" / "training"
        sources = [
            csrc_dir / "forward.cu",
            csrc_dir / "backward.cu",
            csrc_dir / "regularization.cu",
            csrc_dir / "bindings.cpp",
        ]
        extra_inc  = _find_cuda_include()
        extra_flags = ["-O3", "--use_fast_math"] + [f"-I{p}" for p in extra_inc]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out",     default=None,
                   help="Checkpoint output directory. If unset, derived automatically "
                        "from --volume's parent folder name and filename stem, e.g. "
                        "processed_data/complex/image_z14_y11_x17.tif -> "
                        "outputs/complex/image_z14_y11_x17.")
    ...
    # Auto-derive the checkpoint directory from the input block's own folder
    # name plus the volume's filename stem (e.g.
    # processed_data/complex/image_z14_y11_x17.tif ->
    # outputs/complex/image_z14_y11_x17) when --out wasn't given explicitly
    # (CLI or YAML).
    if args.out is None:
        volume_path = Path(args.volume).resolve()
        block_name = volume_path.parent.name
        args.out = str(Path("outputs") / block_name / volume_path.stem)
