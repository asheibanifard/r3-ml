...
int main(int argc, char** argv) {
    ...
    double dvr_total_time = 0.0, baked_total_time = 0.0, rast_total_time = 0.0;
    // GPU-only time via CUDA events: wall-clock timing above (std::chrono around
    // cudaDeviceSynchronize) also includes CPU-side kernel-launch dispatch and the
    // synchronize round-trip -- measured directly (see fps_diagnostic investigation)
    // to be 10-250x LARGER than actual GPU kernel execution time for this cheap,
    // cache-resident DVR workload. That overhead is roughly fixed per launch, so it
    // swamps the real, resolution-dependent GPU cost at small/medium screen sizes.
    // CUDA events timestamp the GPU's own execution directly, isolating the number
    // that actually answers "how fast is this rendering algorithm" from "how fast can
    // the CPU dispatch a kernel and wait for it."
    cudaEvent_t ev_dvr0, ev_dvr1, ev_baked0, ev_baked1, ev_rast0, ev_rast1;
    cudaEventCreate(&ev_dvr0); cudaEventCreate(&ev_dvr1);
    cudaEventCreate(&ev_baked0); cudaEventCreate(&ev_baked1);
    cudaEventCreate(&ev_rast0); cudaEventCreate(&ev_rast1);
    float dvr_gpu_ms = 0.0f, baked_gpu_ms = 0.0f, rast_gpu_ms = 0.0f;

    printf("=== Render sweep: %d frames, %dx%d ===\n", N_FRAMES, IMAGE_W, IMAGE_H);
    for (int frame = 0; frame < N_FRAMES; ++frame) {
        float yaw = 360.0f * frame / N_FRAMES;
        Camera cam = make_camera(yaw);
        char path[512];
        float ms;

        // --- GT: vanilla DVR on the ground-truth voxel grid ---
        auto t0 = std::chrono::high_resolution_clock::now();
        cudaEventRecord(ev_dvr0);
        dvr_render_kernel<<<gridDim2D, blockDim2D>>>(d_target, cam, d_image_gt, IMAGE_W, IMAGE_H);
        cudaEventRecord(ev_dvr1);
        CUDA_CHECK(cudaDeviceSynchronize());
        auto t1 = std::chrono::high_resolution_clock::now();
        dvr_total_time += std::chrono::duration<double>(t1 - t0).count();
        cudaEventElapsedTime(&ms, ev_dvr0, ev_dvr1); dvr_gpu_ms += ms;

        CUDA_CHECK(cudaMemcpy(h_image, d_image_gt, IMAGE_W * IMAGE_H * sizeof(float), cudaMemcpyDeviceToHost));
        snprintf(path, sizeof(path), "%s/gt_%04d.bin", out_dir.c_str(), frame);
        write_frame(path, h_image, IMAGE_W, IMAGE_H);

        // --- Baked: same DVR kernel, but on the reconstructed (baked) grid ---
        auto tb0 = std::chrono::high_resolution_clock::now();
        cudaEventRecord(ev_baked0);
        dvr_render_kernel<<<gridDim2D, blockDim2D>>>(d_baked, cam, d_image_baked, IMAGE_W, IMAGE_H);
        cudaEventRecord(ev_baked1);
        CUDA_CHECK(cudaDeviceSynchronize());
        auto tb1 = std::chrono::high_resolution_clock::now();
        baked_total_time += std::chrono::duration<double>(tb1 - tb0).count();
        cudaEventElapsedTime(&ms, ev_baked0, ev_baked1); baked_gpu_ms += ms;

        CUDA_CHECK(cudaMemcpy(h_image, d_image_baked, IMAGE_W * IMAGE_H * sizeof(float), cudaMemcpyDeviceToHost));
        snprintf(path, sizeof(path), "%s/baked_%04d.bin", out_dir.c_str(), frame);
        write_frame(path, h_image, IMAGE_W, IMAGE_H);

        // --- Reconstruction: tile-based Gaussian rasterizer (live, continuous) ---
        auto t2 = std::chrono::high_resolution_clock::now();
        cudaEventRecord(ev_rast0);
        CUDA_CHECK(cudaMemset(d_tile_counts, 0, n_tiles * sizeof(int)));
        build_tile_lists_kernel<<<blocksG, threadsG>>>(d_rt, N_GAUSSIANS, cam, d_tile_counts, d_tile_gaussian_list,
                                                        IMAGE_W, IMAGE_H, tiles_x, tiles_y);
        CUDA_CHECK(cudaGetLastError());
        render_tiles_kernel<<<tileGridDim, tileBlockDim>>>(d_rt, cam, d_tile_counts, d_tile_gaussian_list,
                                                            d_image_rec, IMAGE_W, IMAGE_H, tiles_x, N_GAUSSIANS, t_far, RENDER_BINS);
        CUDA_CHECK(cudaGetLastError());
        cudaEventRecord(ev_rast1);
        CUDA_CHECK(cudaDeviceSynchronize());
        auto t3 = std::chrono::high_resolution_clock::now();
        rast_total_time += std::chrono::duration<double>(t3 - t2).count();
        cudaEventElapsedTime(&ms, ev_rast0, ev_rast1); rast_gpu_ms += ms;

        CUDA_CHECK(cudaMemcpy(h_image, d_image_rec, IMAGE_W * IMAGE_H * sizeof(float), cudaMemcpyDeviceToHost));
        snprintf(path, sizeof(path), "%s/rec_%04d.bin", out_dir.c_str(), frame);
        write_frame(path, h_image, IMAGE_W, IMAGE_H);

        if (frame % 10 == 0)
            printf("  frame %2d/%d  yaw=%.1f\n", frame, N_FRAMES, yaw);
    }
    cudaEventDestroy(ev_dvr0); cudaEventDestroy(ev_dvr1);
    cudaEventDestroy(ev_baked0); cudaEventDestroy(ev_baked1);
    cudaEventDestroy(ev_rast0); cudaEventDestroy(ev_rast1);

    double dvr_fps = N_FRAMES / dvr_total_time;
    double baked_fps = N_FRAMES / baked_total_time;
    double rast_fps = N_FRAMES / rast_total_time;
    double dvr_gpu_fps = N_FRAMES / (dvr_gpu_ms / 1000.0);
    double baked_gpu_fps = N_FRAMES / (baked_gpu_ms / 1000.0);
    double rast_gpu_fps = N_FRAMES / (rast_gpu_ms / 1000.0);
    printf("\n=== FPS (wall-clock, host-dispatch-overhead-bound) ===\n");
    printf("  GT DVR (voxel grid):                 %.2f FPS\n", dvr_fps);
    printf("  Baked+DVR (reconstructed grid):       %.2f FPS\n", baked_fps);
    printf("  Gaussian rasterizer, tile-based:      %.2f FPS\n", rast_fps);
    printf("=== FPS (GPU-only, CUDA events -- the number that reflects actual rendering cost) ===\n");
    printf("  GT DVR (voxel grid):                 %.2f FPS\n", dvr_gpu_fps);
    printf("  Baked+DVR (reconstructed grid):       %.2f FPS\n", baked_gpu_fps);
    printf("  Gaussian rasterizer, tile-based:      %.2f FPS\n", rast_gpu_fps);

    // Write an FPS summary file for the Python metrics/plotting script.
    {
        std::string fps_path = out_dir + "/fps_summary.txt";
        FILE* f = fopen(fps_path.c_str(), "w");
        fprintf(f, "dvr_fps %.4f\n", dvr_fps);
        fprintf(f, "baked_fps %.4f\n", baked_fps);
        fprintf(f, "rasterizer_fps %.4f\n", rast_fps);
        fprintf(f, "dvr_gpu_fps %.4f\n", dvr_gpu_fps);
        fprintf(f, "baked_gpu_fps %.4f\n", baked_gpu_fps);
        fprintf(f, "rasterizer_gpu_fps %.4f\n", rast_gpu_fps);
    ...
    printf("Run render_outputs.py to assemble mp4s, plots, and the metrics table.\n");
