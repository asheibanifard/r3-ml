    ...
    int repeat_id)
{
    ...
    const int ratio_percent = int(std::lround(
...
int main(int argc, char** argv) {
    try {
        ...
        // RQ1 automatic sweep:
        // 8 payload ratios x 5 independent timing repeats = 40 runs.
        ...
        const int expected_run_count =
            static_cast<int>(retention_ratios.size()) * repeat_count;

        // Prevent accidental duplication of rows from an earlier experiment.
        if (!base_options.csv_output.empty()) {
                ...
                std::filesystem::file_size(csv_path) > 0)
            {
                throw std::runtime_error(
                    ...
                    base_options.csv_output +
                    ". Move or delete it before starting a new automatic sweep."
                );
            }
        }
        ...
        // Load the complete Gaussian scene once. Every payload level is a
        // deterministic nested subset of one fixed shuffled ordering.
        const std::vector<GaussianDisk> full =
            read_gaussians(base_options.input_path);

        constexpr uint32_t subset_seed = 42u;

        // Generate one ordering once and reuse it for every payload ratio and
        // every timing repeat. This removes dependence on the binary's
        // original Gaussian ordering while preserving nested subsets.
        const std::vector<size_t> fixed_order =
            make_fixed_shuffled_order(
                full.size(),
                subset_seed
            );
        ...
        std::cout
            << "RQ1 automatic payload-scalability benchmark\n"
            << "GPU: " << device.name << "\n"
            << "GPU total VRAM bytes: "
            << device.totalGlobalMem << "\n"
            ...
            << expected_run_count << "\n"
            << "Resolution: "
            << base_options.width << " x "
            << base_options.height << "\n"
            << "Depth samples: "
            << base_options.depth_samples << "\n"
            << "Warm-up frames per run: "
            << base_options.warmup_frames << "\n"
            << "Measured frames per run: "
            << base_options.measured_frames << "\n"
            << "Camera position: "
            << base_options.camera_position.x << " "
            << base_options.camera_position.y << " "
            << base_options.camera_position.z << "\n"
            << "Yaw/pitch/roll: "
            << base_options.yaw << " "
            << base_options.pitch << " "
            << base_options.roll << "\n"
            ...
            << base_options.fov_y << "\n"
            << "Render box minimum: "
            << base_options.box.minimum.x << " "
            << base_options.box.minimum.y << " "
            << base_options.box.minimum.z << "\n"
            << "Render box maximum: "
            << base_options.box.maximum.x << " "
            << base_options.box.maximum.y << " "
            << base_options.box.maximum.z << "\n"
            << "Tile size: "
            << TILE_W << " x "
            << TILE_H << "\n"
            << "Mahalanobis cutoff: "
        ...
        for (const double retention_ratio : retention_ratios) {
            // Construct this deterministic subset once and reuse the exact
            // same Gaussian records for all five timing repeats.
            ...
            const double active_payload_mib =
                double(active_payload_bytes) /
                (1024.0 * 1024.0);
                 ...
                 ++repeat_id)
            {
                ...
                // Renderer construction performs allocation, upload,
                // preprocessing, binning, scan, duplication, and sorting
                // before the steady-state timing region.
                Renderer renderer(
                    active,
                    run_options.width,
                    run_options.height,
                ...
                // Time each steady-state frame independently.
                for (int frame = 0;
                ...
                for (const double frame_ms : frame_times_ms) {
                    fps_values.push_back(
                        1000.0 / frame_ms
                    );
                }
                ...
                // Render and download one validation image after timing.
                renderer.render();
                ...
                if (!std::isfinite(output_min) ||
                    !std::isfinite(output_max))
                {
                    throw std::runtime_error(
                        "Rendered output contains a non-finite range."
                    );
                }
        ...
        if (completed_runs != expected_run_count) {
            throw std::runtime_error(
                "Automatic sweep completed an unexpected number of runs."
            );
        }

        std::cout
            << "\nRQ1 automatic sweep completed successfully.\n"
            << "Completed runs: "
            << completed_runs
            << "\n";
